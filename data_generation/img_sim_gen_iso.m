function [imagey,position_para,orientation_par,signal]=img_sim_gen_iso(img_para,sim_para,pmask)
...
muxx_1 = ones(1,molecule_num);
muyy_1 = zeros(1,molecule_num);
muzz_1 = zeros(1,molecule_num);

muxx_2 = zeros(1,molecule_num);
muyy_2 = ones(1,molecule_num);
muzz_2 = zeros(1,molecule_num);

muxx_3 = zeros(1,molecule_num);
muyy_3 = zeros(1,molecule_num);
muzz_3 = ones(1,molecule_num);
...
muxy = zeros(1,molecule_num);
muxz = zeros(1,molecule_num);
muyz = zeros(1,molecule_num);
...
%-------------------------
imgy_1 = bsxfun(@times,XXy,reshape(muxx_1,1,1,molecule_num))...
    + bsxfun(@times,YYy,reshape(muyy_1,1,1,molecule_num))+...
    + bsxfun(@times,ZZy,reshape(muzz_1,1,1,molecule_num));

imgy_2 = bsxfun(@times,XXy,reshape(muxx_2,1,1,molecule_num))...
    + bsxfun(@times,YYy,reshape(muyy_2,1,1,molecule_num))+...
    + bsxfun(@times,ZZy,reshape(muzz_2,1,1,molecule_num));

imgy_3 = bsxfun(@times,XXy,reshape(muxx_3,1,1,molecule_num))...
    + bsxfun(@times,YYy,reshape(muyy_3,1,1,molecule_num))+...
    + bsxfun(@times,ZZy,reshape(muzz_3,1,1,molecule_num));

imgy = imgy_1+imgy_2+imgy_3;
% imgy_sum = zeros(size(pmask));

% for nn=1:molecule_num
%     imgy_sum = imgy_sum + imgy(:,:,nn);
% end

% imgy = imgy./sum(sum(imgy));
...
% output
orientation_par.muxx=muxx_1;
orientation_par.muyy=muyy_1;
orientation_par.muzz=muzz_1;
