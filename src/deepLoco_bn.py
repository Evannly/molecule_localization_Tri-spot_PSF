...
from nets import deepLoco_decoder

class deepLoco_net(object):

    def __init__(self, img_channels=1, psize = 400, cost="deepLoco_error", cost_kwargs={}, **kwargs):

        # basic variables
        self.img_channels = img_channels

        # placeholders for input x and y
        self.x = tf.placeholder("float32", shape=[None, psize, psize, img_channels])
        ...
        self.recons = deepLoco_decoder(self.x, psize=psize)
    ...
    def _lap_kernel(self, x, y, sigma):
        ...
        repeat_param = tf.shape(x)[1]
        matrix1 = tf.tile(tf.expand_dims(x, 2), [1, 1, repeat_param, 1]) # repeat column
        matrix2 = tf.tile(tf.expand_dims(y, 1), [1, repeat_param, 1, 1])

        return tf.exp(-tf.reduce_sum(tf.abs(matrix1-matrix2), 3)/sigma)

    def _batch_dot(self, x1, y, x2):

        repeat_param = tf.shape(x1)[1]
        x1 = tf.tile(tf.expand_dims(x1,2),[1,1,repeat_param])
        z = tf.multiply(x1, y)
        z = tf.reduce_sum(z, 1)
    ...
    def _deepLoco_loss2(self, y_pred, y_true):
        '''concate true and pred'''
        print('deep2')
        ...
        weight = tf.concat([weight_true, weight_pred], axis=1)
        position = tf.concat([position_true,position_pred],axis=1)

        loss = 0.0
        for sigma in range(20,50,10):
            K = self._lap_kernel(position,position,sigma=sigma)
            # print(K.shape)

            s1 = self._batch_dot(weight, K, weight)
            print('s1',tf.shape(s1))

            loss = loss + tf.reduce_sum(s1)
        # print(loss.shape)
        return loss

    def _deepLoco_loss3(self, y_pred, y_true):
        '''use different sigma'''
        print('deep3')
        ...
        kernel_sigma = [64.0, 320.0, 640.0, 1920.0]
    ...
    def _get_cost(self, cost_name, cost_kwargs):
        ...
        if cost_name == "mean_squared_error":
            ...
            loss = self._deepLoco_loss3(flat_recons, flat_truths)
        else:
